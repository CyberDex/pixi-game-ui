...
export class DottedProgress extends Layout {
    ...
    constructor(
        ...
        for (let i = 0; i < options.steps; i++) {
            // create all the dots (blocks) of the progress bar
            ...
            button.onDown.connect(() => {
                // connect button down event to the callback that will change the value of the progress bar
                this.val = i + 1; // set the value of the progress bar to the index of the dot (block) + 1
            });

            dots.push({
                // add the dot (block) to the array of dots (blocks) layout configs list
                id: `dot${i}`, // id of the dot (block)
                content: {
                    // content of the dot (block)
                    bg: {
        ...
        const minusButton = new SmallIconButton( // create a button that will decrease the value of the progress bar
            ...
            () => {
                ...
                if (this._val > 0) {
                    // check if the value of the progress bar is greater than 0
        ...
        const plusButton = new SmallIconButton( // create a button that will increase the value of the progress bar
            ...
            () => {
                // callback for the button press
                if (this.val < options.steps) {
                    // check if the value of the progress bar is less than the number of steps (blocks) of the progress bar
        ...
        super({
            ...
            content: {
                ...
                minusButton: {
                    // minusButton is the id of the layer
                ...
                plusButton: {
                    // plusButton is the id of the layer
    ...
    private updateState() {
        ...
        dots.forEach((dot, i) => {
            // iterate through all the dots (blocks) of the progress bar

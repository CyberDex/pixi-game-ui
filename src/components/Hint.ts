...
export class Hint extends Layout {
    ...
    constructor(
        ...
        super({ // Layout constructor accepts an object with all the config
            content: { // content is an object with all the layers of the hint
    ...
    public async show(force = false) {// force is a boolean that indicates if the hint should be shown without tweening
        if (this.alpha === 1) return; // if the hint is already visible, return

        gsap.killTweensOf(this); // kill all tweens of the hint
        
        if (force) { // if force is true, show the hint without tweening
        ...
        await gsap.to(this, { // tween the hint
    ...
    public async hide(force = false) {// force is a boolean that indicates if the hint should be hidden without tweening
        if (this.alpha === 0) return; // if the hint is already hidden, return

        gsap.killTweensOf(this); // kill all tweens of the hint
        
        if (force) { // if force is true, hide the hint without tweening
            this.alpha = 0; // set alpha to 0
            return;
        }
